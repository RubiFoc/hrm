...
## Last Updated
- Date: 2026-03-20
- Updated by: coordinator + architect + frontend-engineer
...
| `TASK-11-06` | implemented/local-baseline | `/candidate/apply` now provides the compatibility public apply shell, browser checksum calculation, session storage context, and tracking/analysis polling; `/candidate` redirects into it |
...
| `TASK-11-08` | implemented/local-interview-slice | Interview scheduling and candidate registration are now implemented against `docs/project/interview-planning-pass.md`, including HR controls on `/hr`, public token registration on `/candidate/interview/:interviewToken`, and free-mode Google Calendar sync via shared interviewer calendars |
...
| `TASK-06-01` | implemented/local-offer-slice | Offer persistence, staff offer lifecycle APIs, `/hr` offer block, and pipeline guards for `offer -> hired/rejected` are implemented without reopening auth, CORS, route topology, or public candidate transport |
...
- The current approved local diff includes the `TASK-11-08` backend+frontend interview scheduling and candidate-registration slice, including the `/candidate/apply` and `/candidate/interview/:interviewToken` compatibility topology.
...
- The current baseline PR must keep exactly this scope:
  ...
  - HR vacancy/pipeline workspace on `/hr`;
  - browser smoke for staff login + public candidate apply through `/candidate/apply`;
...
- `TASK-05-03/04` is now implemented from `docs/project/interview-feedback-fairness-pass.md`, including staff feedback APIs, the `/hr` workspace feedback block, and the existing `interview -> offer` fairness gate.
...
- The preserved slice rules remain:
  - keep HR interview controls on `/hr`;
  - keep candidate apply on `/candidate/apply` and candidate interview registration on `/candidate/interview/:interviewToken`;
